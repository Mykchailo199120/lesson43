...
function runCodeInTestContext(code, opts, context = (_sharedTestContext => (_sharedTestContext = sharedTestContext) != null ? _sharedTestContext : sharedTestContext = createTestContext())()) {
  ...
  _run = _asyncToGenerator(function* (task) {
    ...
    if (opts.sourceMaps === true) {
      try {
        ...
        var _task$sourceMapFile, _task$sourceMapFile$l;
        if (!process.env.OVERWRITE && task.sourceMap) throw e;
        (_task$sourceMapFile$l = (_task$sourceMapFile = task.sourceMapFile).loc) != null ? _task$sourceMapFile$l : _task$sourceMapFile.loc = task.taskDir + "/source-map.json";
...
function normalizeOutput(code, {
  ...
  const dir = _path.resolve(_path.dirname(__filename), "../../..");
  const symbol = "<CWD>";
  let result = code.trim().replace(new RegExp(escapeRegExp(dir), "g"), symbol);
  if (process.platform === "win32") {
    result = result.replace(new RegExp(escapeRegExp(dir.replace(/\\/g, "/")), "g"), symbol).replace(new RegExp(escapeRegExp(dir.replace(/\\/g, "\\\\")), "g"), symbol);
    if (normalizePathSeparator) {
      result = result.replace(new RegExp(`${escapeRegExp(symbol)}[\\w\\\\/.-]+`, "g"), path => path.replace(/\\\\?/g, "/"));
    }
  }
  {
  ...
  for (const testSuite of suites) {
    ...
    describe(name + "/" + testSuite.title, function () {
      ...
      for (const task of testSuite.tests) {
        ...
        testFn(testTitle, _asyncToGenerator(function* () {
